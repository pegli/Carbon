...
var initBridge = function() {
	if(bridge === undefined) {
		// This will loop in the native APIs so we can mess with them in JS land to prevent un-needed bridge crossings
		bridge = require('appersonlabs.carbon');

		if(ti_properties.Platform.name === 'android') {
			bridge.setCreateWindow(Ti.UI.createWindow);
		}
	}
